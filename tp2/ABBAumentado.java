...
public class ABBAumentado <K extends Comparable<? super K>, V> implements Iterable<K> {
	...
	public Nodo<K,V> encNodo(Nodo<K, V> nodo, K clave) {
		...
		if (clave.compareTo(nodo.clave)<0){
			...
			return encNodo(nodo.izq, clave);
		}else if( clave.compareTo(nodo.clave)>0){
			if(nodo.der!=null && clave.compareTo(nodo.der.clave)==0){
				return nodo;
			}
			return encNodo(nodo.der,clave);
		}else{
			return null;
		}
	}

	public Nodo<K, V> encNodoPadre(Nodo<K, V> nodo, K clave) {
    if (nodo == null) {
        return null;
    }
	this.visitas++;
    if (clave.compareTo(nodo.clave) < 0) {
        ...
        return encNodoPadre(nodo.izq, clave);
    } else if (clave.compareTo(nodo.clave) > 0) {
        if (nodo.der != null && clave.compareTo(nodo.der.clave) == 0) {
            return nodo;
        }
        return encNodoPadre(nodo.der, clave);
    } else {
        return null;
    }
}
 */
	...
	public V obtener(K clave) throws ClaveInexistenteException, ClaveNulaException {
		...
		V valor = obtenerRec(this.raiz, clave);

		if (valor == null) {
			throw new ClaveInexistenteException(
				"La clave " + clave + " no existe en el arbol."
			);
		}
